...
def detect_changes(old_record: Dict[str, Any], api_data: Dict[str, Any], columns: List[str]) -> Tuple[bool, Dict[str, Any], Dict[str, Dict[str, Any]]]:
    ...
    changes_details shows what specifically changed for console output.
    """
    has_changes = False
    all_values = {}
    changes_details = {}
    ...
    for source_col in columns:
        ...
        if source_col == 'image':
            ...
            if old_norm != new_norm:
                ...
                logger.debug(f"Change detected in {source_col}: {old_value} -> {clean_new_value}")
        else:
    ...
    try:
        ...
        for idx, record in enumerate(valid_records, 1):
            ...
            if has_changes:
                ...
                print(f"\n{'='*60}")
                print(f"CHANGES DETECTED for Apple ID {apple_id}")
                print(f"{'='*60}")
                for col, diff in changes_details.items():
                    old_display = diff['old'] if diff['old'] else '(None)'
                    new_display = diff['new'] if diff['new'] else '(None)'
                    print(f"  {col}:")
